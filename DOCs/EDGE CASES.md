...
* **Dashboard:** When a row’s current state is `Mixed`, **Space** toggles only the desired **Start** vs **Stop** target (`Ready` vs `Stopped`).
* **Mixed triage aid:** On the App Workloads tab, press **`\``** to cycle detail modes (`None` -> `MixedOnly` -> `All`). In detail modes, the dashboard prints service/executable runtime status per workload so operators can quickly identify which checks are causing `Mixed`.
